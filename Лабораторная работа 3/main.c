...
{
    float HeroVertex[] = {obj.x-80,obj.y-80,obj.x+80,obj.y-80,obj.x+80,obj.y+80,obj.x-80,obj.y+80};
...
void Hero_Move(Hero *obj)
{
    ...
    if (GetKeyState(VK_UP)<0 && (obj->y>432))
    {
        TypeOfAnim=1;
...
{
    ChooseBTN(0,3);
    Menu_AddButton("Start",181*2,10*2,150*2,30*2,2*2);
    Menu_AddButton("Animations",181*2,50*2,150*2,30*2,2*2);
    Menu_AddButton("Quit",181*2,90*2,150*2,30*2,2*2);
    Menu_AddButton("Animation1",181*2,10*2,150*2,30*2,2*2);
    Menu_AddButton("Animation2",181*2,50*2,150*2,30*2,2*2);
    Menu_AddButton("Animation3",181*2,90*2,150*2,30*2,2*2);
    Menu_AddButton("Back",181*2,130*2,150*2,30*2,2*2);
    Menu_AddButton("Back",0,0,100*2,30*2,2*2);
}
...
void InitGame()
{
    Hero_Init(&Robo,height/2,width/2,0.5,0.5);
    ChooseBTN(7,8);
    currentframe=0;
    TypeOfAnim=0;
    anim=1;
...
void ShowTexture(TextureVertex)
{
    ...
    glPushMatrix();
        ...
        static float spriteXsize=640; //переменные с размерами текстуры и отдельного кадра
