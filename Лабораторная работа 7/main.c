...
void Init()
{
    glEnable(GL_BLEND); //включение смешивани€ цветов
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);  //настройка смешивани€ цветов (цвет источника, цвет приемщика)
...
void ShowWord()
{
    glEnableClientState(GL_VERTEX_ARRAY);
        ...
        for (int i = -n; i < n; i++)
        {
            ...
            {
                glPushMatrix();
                    if ((i+j) % 2 == 0) glColor3f(0.0, 0.6, 1);
                    else glColor3f(0.3, 0.0, 0.7);
...
void DrawPrism(const int n)
{
    ...
    const int vn = 2*n; //кол-во вершин

    float vertieces[3*vn]; //нормали
    ...
    for (int i = 0; i < n; i++) {
        ...
        //вершины верхнего основани€
        vertieces[6*i + 0] = c/2;
        vertieces[6*i + 1] = s/2;
        vertieces[6*i + 2] = 1.0;

        //вершины нижнего основани€
        vertieces[6*i + 3] = c;
        vertieces[6*i + 4] = s;
        vertieces[6*i + 5] = -1.0;
    }
                   ...
                   int nCmdShow)
{
    ...
    glEnable(GL_COLOR_MATERIAL);    //активаци€ освещени€
    glEnable(GL_NORMALIZE); //нормализаци€ вектора
    ...
    while (!bQuit)
    {
        ...
        else
        {
            ...
            glPushMatrix();
                MoveCamera();
                ShowWord();
                glPushMatrix(); //вложенна€ матрица
                    glRotatef(24,0,0,1);
                    glRotatef(theta,0,1,0);
                    float position[] = {0,0,1,0};//вектор освещение сверху вниз
                    glLightfv(GL_LIGHT0, GL_POSITION, position);

                    glTranslatef(0,0,40);   //положение освещени€
                    glScalef(1,1,1);    //размер освещени€
                    glColor3f(1,1,1);   //цвет освещени€
                    Draw();
                glPopMatrix();
                glColor3f(0,0,1);
                DrawCube();
